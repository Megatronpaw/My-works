...
public class QuestionRepository(AppDbContext appDbContext, IMapper mapper, ILogger<QuestionRepository> logger) : IQuestionRepository
{
    public async Task<IEnumerable<QuestionDto>> GetAllAsync()
    {
        ...
        var questions = await appDbContext.Questions.AsNoTracking().ToListAsync();
    ...
    public async Task UpdateAsync(QuestionDto questionDto)
    {
        ...
        var question = await appDbContext.Questions.FirstOrDefaultAsync(question => question.Id == questionDto.Id);
    ...
    public async Task DeleteAsync(int id)
    {
        ...
        var question = await appDbContext.Questions.FirstOrDefaultAsync(question => question.Id == id);
        ...
        appDbContext.Questions.Remove(question);
        await appDbContext.SaveChangesAsync();
